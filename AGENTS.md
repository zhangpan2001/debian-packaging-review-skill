...
- 缺少 `README.source` 说明复杂补丁或 repack 原因
...
```bash
cp AGENTS.md /path/to/your/package/
...
```text
请按照 AGENTS.md 中的 Debian 入库整改 Skill，审查当前源码包。
先不要直接大改，先输出必须修复项和建议修复项。
重点检查 debian/control、debian/copyright、debian/rules、lintian、sbuild、piuparts、autopkgtest。
```

如果已经有日志，可以输入：

```text
请根据下面的 lintian / sbuild / piuparts 日志，按照 AGENTS.md 的 Debian 入库整改规则，逐项给出最小修改方案。
```
...
准备提交 Debian mentors / RFS 前，建议这样问：
